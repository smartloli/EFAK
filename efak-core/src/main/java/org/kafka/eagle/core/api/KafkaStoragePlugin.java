...
public class KafkaStoragePlugin {
    ...
    public Properties buildAdminClientProps(KafkaClientInfo clientInfo) {
        ...
        local.put(CommonClientConfigs.BOOTSTRAP_SERVERS_CONFIG, clientInfo.getBrokerServer());
        local.put(CommonClientConfigs.REQUEST_TIMEOUT_MS_CONFIG, 10_000);
        local.put(CommonClientConfigs.DEFAULT_API_TIMEOUT_MS_CONFIG, 15_000);
        local.put(CommonClientConfigs.CONNECTIONS_MAX_IDLE_MS_CONFIG, 300_000);
        if (clientInfo.isSasl()) {
            applySaslConfig(local, clientInfo);
        }
        if (clientInfo.isSsl()) {
            applySslConfig(local, clientInfo);
        }
        return local;
    }
