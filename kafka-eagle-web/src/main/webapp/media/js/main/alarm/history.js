$(document).ready(function () {
    ...
    // alarm crontab
    $(document).on('click', 'a[name=alarm_cluster_crontab]', function () {
        var href = $(this).attr("href");
        var id = href.split("#")[1];
        var type = href.split("#")[2].split("/")[0];
        $("#ke_alarm_cluster_id_crontab").val(id);
        $("#ke_alarm_cluster_id_type").val(type);

        $('#alarm_cluster_crontab').modal({
            backdrop: 'static',
            keyboard: false
        });
        $('#alarm_cluster_crontab').modal('show').css({
            position: 'fixed',
            left: '50%',
            top: '50%',
            transform: 'translateX(-50%) translateY(-50%)'
        ...
        $.ajax({
            type: 'get',
            dataType: 'json',
            url: '/alarm/crontab/get/' + id + '/' + type + '/ajax',
            success: function (datas) {
                if (datas != null) {
                    $("#ke_alarm_cluster_name_crontab").val(datas.crontab);
                }
            }
        });
