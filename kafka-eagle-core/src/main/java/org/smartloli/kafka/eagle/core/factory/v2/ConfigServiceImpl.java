...
/**
 * Implements {@link ConfigService} all method.
 *
 * @author jeff.
 */
public class ConfigServiceImpl implements ConfigService {

	private final String CONFIG_USERS_PATH = "/config/users";
     ...
     */
	@Override
    public long getUserList(String clusterAlias) {
        long count = 0;
        KafkaZkClient zkc = kafkaZKPool.getZkClient(clusterAlias);
        try {
            if (zkc.pathExists(CONFIG_USERS_PATH)) {
                Seq<String> path = zkc.getChildren(CONFIG_USERS_PATH);
                List<String> l = JavaConversions.seqAsJavaList(path);
                count = l.size();
            }
        } catch (Exception e) {
