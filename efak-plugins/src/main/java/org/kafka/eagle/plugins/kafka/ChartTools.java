...
public class ChartTools {
    ...
    public static JSONArray generateTopologyData(List<ConsumerGroupInfo> consumerGroupInfos) {
        ...
        for (ConsumerGroupInfo consumerGroupInfo : consumerGroupInfos) {
            ...
            if (offset >= KConstants.Consumer.TOPOLOGY_SIZE) {
                ...
                if (StrUtil.isNotBlank(consumerGroupInfo.getTopicName())) {
                    flag = true;
                    topicNode.put("x", baseX + offset * n * 400);
                    topicNode.put("y", 600);
    ...
    public static JSONArray generateTopologyDataLine(Long size) {
        JSONArray array = new JSONArray();
        if (size == 0) {
            return array;
        }
        JSONObject object = new JSONObject();
        object.put("coords", JSON.parseObject("[[500, 1000],[500, 800]]", new TypeReference<List<List<Integer>>>() {
        }));
        array.add(object);
        if (size == 1) {
            object = new JSONObject();
            object.put("coords", JSON.parseObject("[[500, 800],[500, 600]]", new TypeReference<List<List<Integer>>>() {
            }));
            array.add(object);
            object = new JSONObject();
            object.put("coords", JSON.parseObject("[[500, 600],[500, 300]]", new TypeReference<List<List<Integer>>>() {
            }));
            array.add(object);
        } else if (size == 2) {
            object = new JSONObject();
            object.put("coords", JSON.parseObject("[[500, 800],[100, 800],[100, 600]]", new TypeReference<List<List<Integer>>>() {
            }));
            array.add(object);
            object = new JSONObject();
            object.put("coords", JSON.parseObject("[[500, 800],[900, 800],[900, 600]]", new TypeReference<List<List<Integer>>>() {
            }));
            array.add(object);
            object = new JSONObject();
            object.put("coords", JSON.parseObject("[[100, 600],[500, 300]]", new TypeReference<List<List<Integer>>>() {
            }));
            array.add(object);
            object = new JSONObject();
            object.put("coords", JSON.parseObject("[[900, 600],[500, 300]]", new TypeReference<List<List<Integer>>>() {
            }));
            array.add(object);
        } else if (size == 3) {
            object = new JSONObject();
