$(document).ready(function () {
    $("#efak_alarm_common_tab").dataTable({
        "bSort": false,
        "bLengthChange": false,
        "bProcessing": true,
        "bServerSide": true,
    ...
    $(document).on('click', 'a[name=alarm_cluster_remove]', function () {
        var href = $(this).attr("href");
        var id = href.split("#")[1].split("/")[0];
        $("#efak_alarm_cluster_remove_content").html("<p class='alert alert-danger'>Are you sure you want to delete id [<strong>" + id + "</strong>] ?<p>");
        $("#efak_alarm_cluster_remove_footer").html("<a href='/alarm/history/" + id + "/del' class='btn btn-danger'>Remove</a>");
    ...
    function select2common(id, url, placeholder) {
        return $("#" + id).select2({
            ...
            theme: 'bootstrap4',
            dropdownParent: $("#alarm_cluster_modify"),
            ajax: {
                url: url,
                dataType: 'json',
                delay: 250,
